...
export default function BorrowerPage() {
  ...
  const { submitFactors, isProcessing, message, borrowerHasSubmitted, hasContract } = useCreditScoreEngine();
  ...
  const previewGrade = preview >= 8500 ? "Excellent" : preview >= 7000 ? "Good" : preview >= 5000 ? "Fair" : "Poor";
  const previewColor = preview >= 7500 ? "text-emerald-400" : preview >= 5000 ? "text-amber-400" : "text-red-400";
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-10 text-center">
          ...
          <p className="mt-2 text-sm text-gray-400">You need a connected wallet to submit encrypted factors.</p>
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] px-4 py-12 text-white">
      <div className="mx-auto max-w-2xl">
        {/* Header */}
        <div className="mb-8">
          <p className="text-sm text-[#FFD208] font-semibold uppercase tracking-widest">Borrower Portal</p>
          <h1 className="mt-2 text-3xl font-extrabold">Simulated Oracle Data Input</h1>
          <p className="mt-2 text-gray-400 text-sm leading-relaxed">
            In a production environment, these factors would be pulled automatically from an encrypted credit bureau
            API. For this hackathon demo, adjust the sliders below to simulate your profile. Your values are{" "}
            <span className="text-[#FFD208] font-semibold">encrypted locally with fhevmjs</span> before being sent
            on-chain — the raw numbers never leave your browser.
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-[#FFD208]/30 bg-[#FFD208]/5 p-6">
          <p className="text-xs text-gray-400 uppercase tracking-widest">Local score preview (not sent on-chain)</p>
          <div className="mt-2 flex items-end gap-3">
            <span className={`text-5xl font-extrabold tabular-nums ${previewColor}`}>{preview}</span>
            <span className={`mb-1 text-lg font-semibold ${previewColor}`}>/ 10 000 · {previewGrade}</span>
          </div>
          <p className="mt-2 text-xs text-gray-500">ph×40 + (100−dti)×30 + ca×15 + (100−util)×15</p>
        </div>

        {/* Encryption notice */}
        <div className="mt-4 rounded-xl border border-purple-500/20 bg-purple-500/5 p-4 text-xs text-purple-300">
          🔒 When you click Submit, fhevmjs will encrypt all four values into <code className="font-mono">euint64</code>{" "}
          ciphertexts and generate a ZK input-proof covering them. Only the encrypted handles + proof are broadcast to
          Sepolia.
        </div>
