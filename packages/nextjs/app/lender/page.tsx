...
function ScoreMeter({ score }: { score: number }) {
  ...
  const color =
    score >= 7500 ? "#10b981" : score >= 5000 ? "#FFD208" : "#ef4444";
  const grade =
    score >= 8500 ? "Excellent" : score >= 7000 ? "Good" : score >= 5000 ? "Fair" : "Poor";
    ...
    <div className="min-h-screen bg-[#0D0D0D] px-4 py-12 text-white">
      <div className="mx-auto max-w-xl">
        ...
        <p className="mt-2 text-gray-400 text-sm leading-relaxed">
          Enter a borrower address you are authorised for. The contract returns the encrypted score
          handle; the Zama KMS then decrypts it client-side — only the final number is revealed.
        </p>
        ...
        <div className="mt-4 rounded-xl border border-purple-500/20 bg-purple-500/5 p-4 text-xs text-purple-300">
          🧮 The contract calls <code>FHE.ge(score, threshold)</code> for threshold checks —
          returning an <code>ebool</code> — and can return the full encrypted score handle for
          authorised lenders. Raw factors are <strong>never</strong> accessible.
        </div>
        ...
        {lenderClearScore !== undefined && (
          <div className="mt-4 rounded-2xl border border-[#FFD208]/30 bg-[#FFD208]/5 p-6">
            <p className="text-sm font-semibold text-[#FFD208] uppercase tracking-widest mb-1">
              Decrypted Score
            </p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 text-center">
              {[
                ...
                >
                  <p className="font-bold">{t.pass ? "✓" : "✗"} {t.value}</p>
